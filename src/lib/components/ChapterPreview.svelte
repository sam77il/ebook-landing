<script>
	...
	let selectedChapterContent = $derived(
		chapters.find((chapter) => chapter.number === selectedChapter)
	);

	// let selectedChapterContent = $derived.by(() => {
	// 	return {
	// 		strapline: chapters[selectedChapter - 1].strapline,
	// 		excerpt: chapters[selectedChapter - 1].excerpt
	// 	};
	// });
</script>
...
<section class="landing-page-section">
	...
	<div class="chapter-container">
			...
			{#each chapters as chapter}
				<li>
					<button
						...
						onclick={() => (selectedChapter = chapter.number)}
					>
		...
		<div class="chapter-info">
			<h3 class="chaspter-strapline italic mb-s">{selectedChapterContent.strapline}</h3>
			<p>{selectedChapterContent.excerpt}</p>
		</div>
	</div>
</section>
